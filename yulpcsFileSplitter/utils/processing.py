...
import requests
from utils.storage_helpers import blob_exists, create_container, download_blob, insert_review, query_container_exists, query_review, query_status, update_container_query, update_review, update_status, upload_blob
import glom
import time
...
def get_welp_business_list(element, TABLE_NAME, API_KEY, API_ENDPOINT, connection_string, url):
    ...
    for item in businesses:
        ...
        #logging.info('review found for location {} rowkey {}'.format(review['PartitionKey'], review['Key']))
        row_key = business_id #+'_'+str(msg['offset'])
        #review = query_review('reviews', business_id, row_key, connection_string)
...
def get_welp_reviews(element, TABLE_NAME, connection_string, api_endpoint, sql_connection_string):
    #logging.info(element)
    #return
    try:
        yelp_spider = YelpSpider()
        ...
        insert_review(element.get('id'), str.lower(element.get('state')), element, details, sql_connection_string)
